// Comprehensive list of all Indian States, Union Territories, and their Districts
// Based on Census 2011 and updated administrative data
...
export const indianStatesAndDistricts: State[] = [
  {
    name: "Andhra Pradesh",
  ...
  {
    name: "Mizoram",
    districts: ["Aizawl", "Champhai", "Kolasib", "Lawngtlai", "Lunglei", "Mamit", "Saiha", "Serchhip", "Khawzawl", "Hnahthial", "Saitual"]
  },
  {
    name: "Nagaland",
...
export const getAllStates = (): string[] => {
  return indianStatesAndDistricts.map(state => state.name);
};

// Helper function to get districts by state
export const getDistrictsByState = (stateName: string): string[] => {
  const state = indianStatesAndDistricts.find(s => s.name === stateName);
  return state ? state.districts : [];
};

// Helper function to get all districts (flattened)
export const getAllDistricts = (): string[] => {
  return indianStatesAndDistricts.flatMap(state => state.districts);
};
...
// Helper function to get state by district
export const getStateByDistrict = (districtName: string): string | null => {
